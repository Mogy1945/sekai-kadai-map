...
(function () {
  ...
  // ジェスチャー中の倍率量子化 (2%刻み)。テキスト再レイアウト等を毎フレーム→クロス時のみに減らす
  const qk = k => gesturing ? Math.exp(Math.round(Math.log(k) / 0.02) * 0.02) : k;
  ...
  function frame(now) {
    ...
    // ジェスチャー中(パン/ホイール/ピンチ)は、倍率依存の値を量子化して
    // 全テキストの再レイアウトと全ノードのscale書き換えを間引く (終了時に精密値へスナップ)
    gesturing = (dragState && dragState.type === 'pan') || pinch || (now - lastWheelAt < 250);
    camKq = qk(cam.k);
    ...
    const fsv = (13 * clamp(camKq, 0.8, 1)).toFixed(1) + 'px';
  ...
  function renderNodeDom(nd, cp, netX, netY, t, mc) {
    ...
    if (t > 0) {
      ...
      s = lerp(worldR / nd.r, camKq, t); // ジェスチャー中は量子化倍率でscale書き換えを間引く
    } else {
      s = worldR / nd.r;
    }
    nd.sx = x; nd.sy = y;
